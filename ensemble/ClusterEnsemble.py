...
def to_pymetis_format(adj_mat):
    ...
    for i in range(n_rows):
        ...
        adjncy += list(idx_col)
        eweights += list(np.array(val.data)[0])
...
def hgpa(base_clusters, nclass, random_state):
    ...
    config_path = '../km1_kKaHyPar_sea20.ini'
...
def calc_objective(base_clusters, consensus_cluster):
    ...
    for bc in base_clusters:
        ...
        objv += normalized_mutual_info_score(
            consensus_cluster[idx], bc[idx], average_method='arithmetic')
...
def cluster_ensembles(
    ...
    if solver == 'cspa':
        ...
        ce_solvers = {'hgpa': hgpa, 'mcla': mcla, 'hbgf': hbgf, 'cspa': cspa, 'nmf': nmf}
        ...
        for name, ce_solver in ce_solvers.items():
            if ce_solver == cspa or ce_solver == hbgf:
                label_pred = ce_solver(base_clusters, nclass)
            else:
                label_pred = ce_solver(base_clusters, nclass, random_state)
            # objv = calc_objective(base_clusters, label)
            objv = normalized_mutual_info_score(label_true, label_pred, average_method='arithmetic')
            # objv = metrics.silhouette_score(df, label)
            # if verbose:
            #     print('        -', name, ':', objv)
            if verbose:
                # print(name, ':', objv)
...
# label_pred = cluster_ensembles(base_clusters, solver='all', verbose=True)
# nmi_score = normalized_mutual_info_score(label_true, label_pred, average_method='arithmetic')
# print("nmi: ", nmi_score)
...
if __name__ == '__main__':
    ...
    label_true = np.array([1, 1, 1, 2, 2, 3, 3])
    label_pred = cluster_ensembles(base_clusters, solver='mcla', verbose=True)
    nmi_score = normalized_mutual_info_score(label_true, label_pred, average_method='arithmetic')
    print("nmi: ", nmi_score)